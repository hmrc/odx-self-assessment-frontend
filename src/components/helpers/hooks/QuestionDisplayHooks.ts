...
export default function useIsOnlyField(callerDisplayOrder = null, refreshTrigger = null) {
  ...
  useEffect(() => {
    ...
    // Otherwise, use the Assignment context's singleQuestion page value, to fall back to the original logic (checking number of editable fields);
      ...
      const pageHasIgnoredField = PCore.getStoreValue('pageHasIgnoredField', '', 'app');

      const context = PCore.getContainerUtils().getActiveContainerItemName(
        `${PCore.getConstants().APP.APP}/primary`
      );
      const workareaContext = PCore.getContainerUtils().getActiveContainerItemContext(
        `${context}/workarea`
      );

      const editableFields = PCore.getFormUtils().getEditableFields(workareaContext);
      const editableFieldsCount =
        editableFields.length === 1 && editableFields[0].label === 'GoBack'
          ? 0
          : editableFields.length;

      if (editableFieldsCount === 1 && !pageHasNonEditableField && pageHasIgnoredField) {
        defaultOnlyFieldDetails.isOnlyField = true;
...
function registerIgnoredField(ignoredCondition = true) {
  useEffect(() => {
    if (ignoredCondition && !PCore.getStoreValue('pageHasIgnoredField', '', 'app')) {
      PCore.getStore().dispatch({
        type: 'SET_PROPERTY',
        payload: {
          reference: 'pageHasIgnoredField',
          value: true,
          context: 'app',
          isArrayDeepMerge: true
        }
